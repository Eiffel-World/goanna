...
feature
	...
	new_connection_socket : WIRE_DUMP_SERVING_SOCKET is
	...
	multiplex_read_callback is
		...
		do
			...
			if socket /= Void then
				...
				log_hierarchy.category (Internal_category).error ("Server socket error: " 
					+ socket_multiplexer.last_socket_error_code.out + ","
					+ socket_multiplexer.last_extended_socket_error_code.out + " read_bytes=" 
					+ socket_multiplexer.managed_read_count.out)
			end
		end

end -- class WIRE_DUMP_SERVER_SOCKET
