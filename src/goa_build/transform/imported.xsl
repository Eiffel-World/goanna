...
<!--
     	description: "Flatten a Relax NG grammer to inline referenced Include statements"
		     "Step 4: Create a file containing imported namespaces and references for imported elements"
...
<xsl:variable name="first_element" select="(//rng:element)[1]" />
<xsl:variable name="name_of_first_element" select="$first_element/@name" />
<xsl:variable name="cleaned_name_of_first_element" select="substring-before ($name_of_first_element, ':')" />
...
<xsl:template match="rng:include">
	<xsl:variable name="prefix" select="substring-before (@href, '.')" />
