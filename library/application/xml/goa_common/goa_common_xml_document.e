...
feature -- Adding Elements
	...
	add_text_item_element (choice_N2_name_code: INTEGER; choice_N2_value: STRING; text_to_add: STRING) is
		...
		require
			...
			is_valid_choice_N2_name_code: choice_N2_name_code /= xml_null_code implies (create {ARRAY[INTEGER]}.make_from_array (<<class_attribute_code, span_attribute_code>>)).has (choice_N2_name_code)
			valid_value_if_class: choice_N2_name_code /= Void and then choice_N2_name_code = class_attribute_code implies is_valid_attribute_value (class_attribute_code, choice_N2_value)
			valid_value_if_span: choice_N2_name_code /= Void and then choice_N2_name_code = span_attribute_code implies is_valid_attribute_value (span_attribute_code, choice_N2_value)
		deferred
		ensure
	...
	add_hyperlink_element (new_class: STRING; new_url: STRING; text_to_add: STRING) is
		...
		deferred
			...
			current_element_unchanged: current_element_code = old current_element_code
			open_element_count_unchanged: element_stack.count = old element_stack.count
			current_element_contents_updated: current_element_contents.count = (old current_element_contents.count + 1) and current_element_contents @ current_element_contents.upper = hyperlink_element_code
		end
